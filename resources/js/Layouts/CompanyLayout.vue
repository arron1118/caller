    ...
    <a-layout has-sider>
        <a-layout-sider :collapsed="collapsed" :trigger="null" :class="'w-52 h-screen top-0 left-0 bottom-0'" :style="{ position: 'fixed' }" theme="dark" collapsible>
            ...
            <a-menu :selectedKeys="selectedKeys" theme="dark" mode="inline">
                <a-menu-item v-for="item in $page.props.menus.company" :key="item.url" :class="{ 'text-blue-500': route().current(item.url) }" :title="item.title">
                    <template #icon>
                    ...
                    <template #overlay>
                        <a-menu @click="onClick">
                            <a-menu-item key="1">
                                ...
                                <Link :href="route('company.logout')">
                                    个人资料
                                </Link>
                            </a-menu-item>
                            <a-menu-item key="2">
